...
/*
	...
	$Id: user.php 759 2008-11-21 09:20:02Z zhaoxiongfei $
*/
...
class usermodel {
	...
	function delete_useravatar($uidsarr) {
		...
		foreach((array)$uidsarr as $uid) {
			unlink($this->_uc_avatar($uid, 'small', TRUE));
			unlink($this->_uc_avatar($uid, 'middle', TRUE));
			unlink($this->_uc_avatar($uid, 'big', TRUE));
		}		
	}
	...
	function _uc_avatar($uid, $size = '', $returnsrc = FALSE) {
		...
		$dir2 = substr($uid, 3, 2);
		$dir3 = substr($uid, 5, 2);
		return $returnsrc ? UC_ROOT.'data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2)."_avatar_$size.jpg" : '<img style="vertical-align:middle" src="'.UC_API.'/data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2)."_avatar_$size.jpg?".time().'" onerror="this.onerror=null;this.src=\''.UC_API.'/images/noavatar_'.$size.'.gif\'">';
	}
