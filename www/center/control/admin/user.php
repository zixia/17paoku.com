...
/*
	...
	$Id: user.php 845 2008-12-08 05:36:51Z zhaoxiongfei $
*/
...
class control extends adminbase {
	...
	function onls() {
		...
		foreach($userlist as $key => $user) {
			$user['smallavatar'] = $this->_uc_avatar($user['uid'], 'small');
			$userlist[$key] = $user;
		}
	...
	function onedit() {
		...
		$user['bigavatar'] = $this->_uc_avatar($uid, 'big');
...
/*		} elseif($username != $_ENV['user']->replace_badwords($username)) {
	...
	function _uc_avatar($uid, $size = '', $returnsrc = FALSE) {
		...
		$dir2 = substr($uid, 3, 2);
		$dir3 = substr($uid, 5, 2);
		return $returnsrc ? UC_ROOT.'data/avatar/'.$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2)."_avatar_$size.jpg" : '<img src="avatar.php?uid='.intval($uid).'&size='.$size.'">';
	}
